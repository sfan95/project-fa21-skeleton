...
def solve(tasks):
    ...
    while time < 1440 and len(tasks) > 0:
        best = max(tasks, key = lambda task: task.get_max_benefit()/task.get_duration())
        ...
        if end <= 1440:
            ...
            profit += newprofit
            tasks = list(filter(
                lambda x: (x.get_late_benefit(end + x.get_duration() - x.get_deadline()) >= 0.5 * newprofit or x.get_deadline() > end)
